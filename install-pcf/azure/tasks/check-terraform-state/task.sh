...
function checkFileExists()
{
  ...
  if [ "$?" -gt "0" ]; then
    ...
    echo "Existing ${TERRAFORM_AZURE_STATEFILE_NAME} file found. Remove the file for this task to pass or proceed to wipe-env or create-infrastructure tasks if this is expected."
    return 1
  fi
}
