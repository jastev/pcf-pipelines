...
db_host=$(echo $output_json | jq --raw-output '.mysql_paas_fqdn.value')
...
ssh -i pcf.pem -o StrictHostKeyChecking=no "ubuntu@${OPSMAN_DOMAIN_OR_IP_ADDRESS}" "mysql -h $db_host -u $mysql_paas_admin_username -p$mysql_paas_admin_password < /tmp/databases.sql"
